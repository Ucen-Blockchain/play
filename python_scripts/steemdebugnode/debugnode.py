...
class DebugNode( object ):
   ...
   def __init__( self, steemd, data_dir, args='', plugins=[], apis=[], steemd_out=None, steemd_err=None ):
      ...
      For all other requests, the python-ucen library should be used.
      ...
      if( not self._data_dir.exists() ):
         raise ValueError( 'data_dir either does not exist or is not a properly constructed ucen data directory' )
   ...
   def debug_generate_blocks( self, count ):
      ...
      `get_dev_key ucen debug`. Do not use this key on the live chain for any reason.
