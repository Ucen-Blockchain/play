...
{
   ucenio::protocol::transaction     tx;
   std::string                        wif;
};
...
struct tx_signing_result
{
   ...
   ucenio::protocol::public_key_type key;
   ucenio::protocol::signature_type  sig;
};
...
int main(int argc, char** argv, char** envp)
{
   ...
   while( std::cin )
   {
      ...
      sres.key = ucenio::protocol::public_key_type( priv_key.get_public_key() );
