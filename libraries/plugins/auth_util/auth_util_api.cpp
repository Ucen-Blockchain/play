...
#include <ucenio/protocol/sign_state.hpp>

#include <ucenio/chain/account_object.hpp>
#include <ucenio/chain/database.hpp>

#include <ucenio/plugins/auth_util/auth_util_api.hpp>
#include <ucenio/plugins/auth_util/auth_util_plugin.hpp>
...
{
   public:
      auth_util_api_impl( ucenio::app::application& _app );
      void check_authority_signature( const check_authority_signature_params& args, check_authority_signature_result& result );

      std::shared_ptr< ucenio::plugin::auth_util::auth_util_plugin > get_plugin();

      ucenio::app::application& app;
};

auth_util_api_impl::auth_util_api_impl( ucenio::app::application& _app ) : app( _app )
{}

std::shared_ptr< ucenio::plugin::auth_util::auth_util_plugin > auth_util_api_impl::get_plugin()
{
...
auth_util_api::auth_util_api( const ucenio::app::api_context& ctx )
{
