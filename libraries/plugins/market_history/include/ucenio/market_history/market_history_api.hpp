...
#include <ucenio/market_history/market_history_plugin.hpp>
...
{
   public:
      market_history_api( const ucenio::app::api_context& ctx );
...
FC_REFLECT( ucenio::market_history::market_ticker,
   (latest)(lowest_ask)(highest_bid)(percent_change)(steem_volume)(sbd_volume) );
FC_REFLECT( ucenio::market_history::market_volume,
   (steem_volume)(sbd_volume) );
FC_REFLECT( ucenio::market_history::order,
   (price)(steem)(sbd) );
FC_REFLECT( ucenio::market_history::order_book,
   (bids)(asks) );
FC_REFLECT( ucenio::market_history::market_trade,
   (date)(current_pays)(open_pays) );

FC_API( ucenio::market_history::market_history_api,
   (get_ticker)
   (get_volume)
   (get_order_book)
   (get_trade_history)
