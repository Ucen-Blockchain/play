#include <ucenio/market_history/market_history_api.hpp>
...
{
   public:
      market_history_api_impl( ucenio::app::application& _app )
         :app( _app ) {}
...
order_book market_history_api_impl::get_order_book( uint32_t limit ) const
{
   ...
   const auto& order_idx = app.chain_database()->get_index< ucenio::chain::limit_order_index >().indices().get< ucenio::chain::by_price >();
...
market_history_api::market_history_api( const ucenio::app::api_context& ctx )
{
