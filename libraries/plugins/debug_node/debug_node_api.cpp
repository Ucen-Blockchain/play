...
#include <ucenio/app/api_context.hpp>
...
#include <ucenio/plugins/debug_node/debug_node_api.hpp>
#include <ucenio/plugins/debug_node/debug_node_plugin.hpp>

namespace ucenio { namespace plugin { namespace debug_node {
...
class debug_private_key_storage : public private_key_storage
{
   public:
      ...
      virtual void maybe_get_private_key(
         ...
         const ucenio::chain::public_key_type& pubkey,
         const std::string& account_name
         ) override;

      std::string                dev_key_prefix;
      std::map< ucenio::chain::public_key_type, fc::ecc::private_key > key_table;
};

class debug_node_api_impl
{
   public:
      debug_node_api_impl( ucenio::app::application& _app );
      ...
      fc::optional< ucenio::chain::signed_block > debug_pop_block();
      //void debug_push_block( const ucenio::chain::signed_block& block );
      ucenio::chain::witness_schedule_object debug_get_witness_schedule();
      ucenio::chain::hardfork_property_object debug_get_hardfork_property_object();
      ...
      std::shared_ptr< ucenio::plugin::debug_node::debug_node_plugin > get_plugin();

      ucenio::app::application& app;
...
)
{
   ...
   if( (pubkey == ucenio::chain::public_key_type()) || (gen_pub == pubkey) )
   {
      result = gen_priv;
      return;
   }
...
debug_node_api_impl::debug_node_api_impl( ucenio::app::application& _app ) : app( _app )
{
...
uint32_t debug_node_api_impl::debug_push_blocks( const std::string& src_filename, uint32_t count, bool skip_validate_invariants )
{
   ...
   std::shared_ptr< ucenio::chain::database > db = app.chain_database();
     ...
     )
   {
      ...
      uint32_t skip_flags = ucenio::chain::database::skip_nothing;
      if( skip_validate_invariants )
         skip_flags = skip_flags | ucenio::chain::database::skip_validate_invariants;
      for( uint32_t i=0; i<count; i++ )
      {
         //fc::optional< ucenio::chain::signed_block > block = log.read_block( log.get_block_pos( first_block + i ) );
         uint64_t block_pos = log.get_block_pos( first_block + i );
         if( block_pos == ucenio::chain::block_log::npos )
         {
...
{
   return get_plugin()->debug_generate_blocks( debug_key, count, ucenio::chain::database::skip_nothing, 0, &key_storage );
}
...
{
   return get_plugin()->debug_generate_blocks_until( debug_key, head_block_time, generate_sparsely, ucenio::chain::database::skip_nothing, &key_storage );
}

fc::optional< ucenio::chain::signed_block > debug_node_api_impl::debug_pop_block()
{
   std::shared_ptr< ucenio::chain::database > db = app.chain_database();
   return db->fetch_block_by_number( db->head_block_num() );
}

/*void debug_node_api_impl::debug_push_block( const ucenio::chain::signed_block& block )
{
   app.chain_database()->push_block( block );
}*/

ucenio::chain::witness_schedule_object debug_node_api_impl::debug_get_witness_schedule()
{
   return app.chain_database()->get( ucenio::chain::witness_schedule_id_type() );
}

ucenio::chain::hardfork_property_object debug_node_api_impl::debug_get_hardfork_property_object()
{
   return app.chain_database()->get( ucenio::chain::hardfork_property_id_type() );
}
...
std::shared_ptr< ucenio::plugin::debug_node::debug_node_plugin > debug_node_api_impl::get_plugin()
{
...
{
   return app.chain_database()->get( ucenio::chain::hardfork_property_id_type() ).last_hardfork >= hardfork_id;
}
...
debug_node_api::debug_node_api( const ucenio::app::api_context& ctx )
{
...
fc::optional< ucenio::chain::signed_block > debug_node_api::debug_pop_block()
{
   return my->debug_pop_block();
}

/*void debug_node_api::debug_push_block( ucenio::chain::signed_block& block )
{
   my->debug_push_block( block );
}*/

ucenio::chain::witness_schedule_object debug_node_api::debug_get_witness_schedule()
{
   return my->debug_get_witness_schedule();
}

ucenio::chain::hardfork_property_object debug_node_api::debug_get_hardfork_property_object()
{
