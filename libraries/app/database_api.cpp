...
#include <ucenio/app/database_api.hpp>

#include <ucenio/protocol/get_config.hpp>
...
{
   public:
      database_api_impl( const ucenio::app::api_context& ctx  );
      ~database_api_impl();
      ...
      ucenio::chain::database&                _db;
      std::shared_ptr< ucenio::follow::follow_api > _follow_api;
...
database_api::database_api( const ucenio::app::api_context& ctx )
   : my( new database_api_impl( ctx ) ) {}

database_api::~database_api() {}

database_api_impl::database_api_impl( const ucenio::app::api_context& ctx )
   : _db( *ctx.app.chain_database() )
{
   ...
   try
   {
      ...
      _follow_api = std::make_shared< ucenio::follow::follow_api >( ctx );
   }
...
{
   return ucenio::protocol::get_config();
}
...
vector<account_id_type> database_api_impl::get_account_references( account_id_type account_id )const
{
   ...
   const auto& refs = aidx.get_secondary_indexucenio::chain::account_member_index>();
   ...
   return result;*/
   FC_ASSERT( false, "database_api::get_account_references --- Needs to be refactored for ucen." );
}
...
void database_api::set_pending_payout( discussion& d )const
{
   ...
   if( total_r2 > 0 )
   {
      ...
      if( my->_db.has_hardfork( STEEMIT_HARDFORK_0_17__774 ) )
      {
         ...
         vshares = d.net_rshares.value > 0 ? ucenio::chain::util::evaluate_reward_curve( d.net_rshares.value, rf.author_reward_curve, rf.content_constant ) : 0;
      }
      else
         vshares = d.net_rshares.value > 0 ? ucenio::chain::util::evaluate_reward_curve( d.net_rshares.value ) : 0;
